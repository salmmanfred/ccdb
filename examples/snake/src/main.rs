...
let assets = loader::load_from_folder("./ast/".to_string(),".rmap".to_string(),".rsprite".to_string()); // get a folder of maps and sprites 
...
loop {
    f.load_map(map1.clone());// clear the vector
    ...
    cursor::hide_cursor();
    for x in snake.iter(){// push all snake parts 
        f.x.push(x[0]);
        f.y.push(x[1]);
    ...
    if rendui{
        ...
        match ui.buttons[1].get_status(){// getting if a button is pressed down or hoverd over or not 
            ui::button_action::Press =>{
                break;
            }
            _ =>{
    ...
    if cur == speed{// makes sure its not too fast 
        x += change_x;
        y += change_y;
        snake.push([x,y]); 
    }
